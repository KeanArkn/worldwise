...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "cities/loaded":
      return {...state, isLoading: false, cities:localData.cities }
    case "city/created":
    ...
    useEffect(() => {
      async function fetchCities() {
        ...
        try{       
          ...
          // setCities(data);
          dispatch({type:"cities/loaded",payload:localData.cities});
        }catch{
    ...
    const getCity = useCallback (async function getCity(id){
      ...
      dispatch({type: "loading"});
        try{
            ...
            // const data  = await res.json();
            // setCurrentCity(data);
            dispatch({type:"city/loaded",payload:cities.find(city => city.id == id)});
        }catch{
          dispatch({type:"rejected",payload:"Error fetching data"});
        }
    },[currentCity.id])
    ...
    async function createCity(newCity){
      ...
      async function deleteCity(id){
        ...
        try{
            ...
            //     method: "DELETE",})
            // const index = localData.cities.findIndex(city => city.id === id);
            // if(index !== -1) {
            //     localData.cities.splice(index, 1);
            // }
